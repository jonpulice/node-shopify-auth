...
ShopifyAuth.prototype.formatShop = function(shop){
	...
	if (subdomain_regex.test(shop)){
		...
		this.log('formatShop: Shop does not match regex');
		return null;
	}
};
...
ShopifyAuth.prototype.mount = function(options){
	...
	routes.get(options.complete_route, this.verifyHMAC({allow_no_hmac: false}), function(req, res){
		var self = this;
...
ShopifyAuth.prototype.authenticate = function(options) {
	...
	this.log('authenticate: Returning authentication middleware with options: ' + JSON.stringify(options));
...
ShopifyAuth.prototype.verifyHMAC = function(options) {
	...
	return function verifyHMAC(req, res, next){
		...
		if (typeof req.query == 'object' && 'hmac' in req.query) {
			...
			return this.verifyAdminHMAC(req, res, next);
		}

		if (req.headers.hasOwnProperty('x-shopify-hmac-sha256')) {
			//hmac header
			return this.verifyWebhookHMAC(req, res, next);
		}
		...
		if ('allow_no_hmac' in options && options.allow_no_hmac == false) {
			...
			this.log('verifyHMAC: No HMAC to verify. Resuming call chain');
			return next();			
		}
